...
%%% sneaky trick 2, do not uncomment
% for i = 2:size(odom_pose,1)
...
% Create diff pose (gt - odom = diff)
diff_pose = [odom_pose(:,1) gt_pose(:,2:end) - odom_pose(:,2:end)];
...
% % TBD
% % xyz error norm over time vs 2% distance travelled
% % xyz error norm over time vs 2% distance travelled ignoring the 1st estimate
% figure(13);
% subplot(2,1,1), plot(diff_pose(:,1), diff_norm, 'b-', diff_pose(:,1), dist_accum.*0.02, 'r-');
%     grid on, legend('xy error norm', '2% travelled distance');
%     xlabel('time [s]'), ylabel('xy error [m]'), title('Visual Odometry Evaluation - xy error norm');
% subplot(2,1,2), plot(diff_pose(:,1), diff_norm-diff_norm(1), 'b-', diff_pose(:,1), dist_accum.*0.02-dist_accum(1), 'r-');
%     grid on, legend('xy error norm', '2% travelled distance');
%     xlabel('time [s]'), ylabel('xy error [m]'), title('Visual Odometry Evaluation - xy error norm');
% 
